...
class TpMatrix : public PackedMatrix<Real> {
 ...
 public:
  ...
  // pykaldi: template<typename OtherReal, typename std::enable_if<!std::is_same<OtherReal, Real>::value>::type* = nullptr> explicit TpMatrix(const TpMatrix<OtherReal>& orig)
